...
export default function App() {
  ...
  // Captura retorno OAuth do Google
  useEffect(() => {
    const handleOAuthCallback = async () => {
      ...
      // Verifica se o usuário já existe na tabela customizada
      ...
      if (!appUser) {
        // Cria o usuário na tabela customizada
      ...
      const cloudList = await loadFromCloud('somente_enviar_pedido_users');
      const emailsList = Array.isArray(cloudList) ? cloudList : [];
      const canSendOnly = emailsList.includes(googleEmail);

      setIsLoggedIn(true);
      setIsAdmin(appUser.role === 'ADMIN');
      setIsSomenteEnviarPedido(canSendOnly);
      setLoggedInEmail(googleEmail);
      setSomenteEnviarEmails(emailsList);

      localStorage.setItem('isLoggedIn', 'true');
      localStorage.setItem('isAdmin', String(appUser.role === 'ADMIN'));
      localStorage.setItem('isSomenteEnviarPedido', String(canSendOnly));
      localStorage.setItem('loggedInEmail', googleEmail);
      setCurrentView(appUser.role === 'ADMIN' ? 'admin' : 'catalog');
    };
        ...
        <div className="w-full max-w-md">
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-3 mt-6">
              ...
              <span className="text-gray-600 font-mono text-xs uppercase tracking-widest">ou</span>
              <div className="flex-1 h-px bg-gray-800" />
            </div>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 bg-white text-gray-800 font-bold py-3.5 rounded-md hover:bg-gray-100 transition-all shadow-md hover:shadow-lg active:scale-[0.98] mt-2"
            >
